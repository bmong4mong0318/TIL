# What is Shells?
 ...
 Shell은 단순하게 명령어를 실행시키는 macro processor입니다. Macro processor란, 단순한 텍스트나 기호들이 확장된 의미를 갖게 하는 환경에서 기능성을 부여하는 것을 의미합니다. 

 Unix shell은 명령어를 해석할 뿐만 아니라 프로그래밍 언어의 역할도 합니다. 명령어를 해석하는데 있어  Unix shell은 많은 유용한 기능을 지닌 GNU 환경에서 사용자 인터페이스를 제공합니다. GNU 환경의 유용한 기능들을 잘 조합하여  사용할 수 있게 만드는 것은 shell의 프로그래밍적 특성입니다.

 명령어를 포함하고 있는 파일들은 실행될 수 있으며, 파일 자체가 명령어인 파일도 존재할 수 있습니다. 이러한 파일들은 /bin에 있는 많은 명령어와 같은 속성을 갖고 있으며, 사용자나 그룹에게 업무를 자동화할 수 있게 사용자 정의 환경을 만들 수 있도록 합니다.

 Shell은 사용자와 상호작용할 때는, 키보드에 입력된 입력을 받아들이고, 상호작용 하지 않을 때는 파일에서 읽어들인 명령어를 실행합니다.

 Shell은 GNU 명령어를 동기적으로 실행하기도 하고, 비동기적으로 실행하기도 합니다. Shell은 더 많은 입력을 받기 전에 동기 명령이 완료될 때까지 기다립니다. 비동기 명령은 shell이 추가 명령을 읽고 실행하는 동안 shelll과 병렬로 계속 실행됩니다. Redirection은 명령어의 입력과 출력에 섬세한 조작 기능을 부여합니다. 더 나아가, 쉘을 사용하면 명령어를 실행시키는 환경의 많은 내용물을 제어할 수 있게 됩니다.

 쉘은 또한 별도의 유틸리티를 통해 얻을 수 없거나 불편한 기능을 구현하는 작은 내장 명령 집합(내장)을 제공합니다. 예를 들어, cd, break, continue, exec과 같은 명령어는 쉘을 직접 조작하기 때문에 쉘 외부에서 구현하지 못합니다. history, getopts, kill, pwd와 같은 내장 명령어들은 구현될 수 있지만, 굳이 그럴 필요없이 내장 명령어를 사용하는 것이 훨씬 편할 것입니다.

 쉘에서 명령어를 실행시키는 것은 중요하지만, 쉘의 진정한 힘은 내장된 프로그래밍 언어들에 있습니다. 여타 고급 언어들처럼, 쉘도 변수, 흐름 제어 구조, 인용 및 함수를 제공합니다.

 쉘은 프로그래밍 언어를 확장하기보다는 대화형 사용을 위해 특별한 기능을 제공합니다. 이러한 특별한 기능에는 작업 제어, command line 편집, command history, alias가 있습니다.
