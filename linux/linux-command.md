...
- streamlined editor (능률적인)
  ...
  - input format : `sed [option] [sed command]`
    - examples : [https://etloveguitar.tistory.com/47](https://etloveguitar.tistory.com/47) 
        ```bash
        #첫 번째 라인만 출력
        sed -n '1p' richet.txt
...
### **root** 
해당 시스템 전체의 관리권한을 가진 전지전능한 슈퍼유저 (**#** → `root상태일때 표시`)

### **admin**
일반유저, Root가 원하면 관리권한을 줄 수 도 있음
...
### **adduser**
- nput format : `adduser [user01] [group01]`
